<template>
	<div class="pb20 pt20 pl20 pr20">
		<div class="like-edit pl20 pr20">
			<div class="clearfix">
				<div class="label pull-left">恰饭管理</div>
				<!-- <div class="query pa">
				...
				<div class="pull-right edit">
					<el-button @click="editDialog = true" size="small" class="" type="success" icon="el-icon-plus">添加恰饭</el-button>
					<el-button @click="pullData" size="small" class="" type="primary" icon="el-icon-refresh">刷新数据</el-button>
				</div>
			</div>
		</div>
		<div class="table mt20" time="1579091233908">
			<!-- 头部筛选 -->
			<div class="pl20 pr20">
		...
		<el-dialog
			...
			<div class="text-center">
				<div class="row-item clearfix">
					<label class="pull-left" for="">恰饭名称：</label>
					<div class="row-con">
						<div>
				...
				<div class="row-item clearfix">
					<label class="pull-left" for="">恰饭类型：</label>
					<div class="row-con">
						<div>
							<el-select style="width: 100%;" size="small" v-model="target.shape" placeholder="请选择">
								<el-option
									v-for="item in [{v: 'web', i: '网站'}, {v: 'app', i: 'App'}]"
									:key="item.v"
									:label="item.i"
									:value="item.v">
								</el-option>
				...
				<div class="row-item clearfix">
					<label class="pull-left" for="">恰饭图片：</label>
					<div class="row-con">
						<div class="pub-flex">
							<div class="pub-flex1">
							...
							<div class="ml10">
								<el-button @click="$Bus.$emit('changePickerAdvert', {showState: true})" size="small" type="primary">选择图片</el-button>
							</div>
						</div>
					</div>
				</div>
				<div class="row-item clearfix">
					<label class="pull-left" for="">恰饭链接：</label>
					<div class="row-con">
						<div>
				...
				<div class="row-item clearfix">
					<label class="pull-left" for="">备注说明：</label>
					<div class="row-con">
						<div>
							<el-input size="small" v-model="target.note" placeholder="请输入内容"></el-input>
						</div>
					</div>
				</div>
			</div>
		...
		<!-- 侧边图片 -->
		<drawer-advert @select="setDataValue" />
	</div>
</template>
<script>
	import { GetAllMeal, RemoveMeal, AddMeal, UpdateMeal } from '@api/meal'
	import { isDev } from '@utils/tools'
	import DrawerAdvert from './drawer-advert.vue'
	export default {
		components: {
			DrawerAdvert,
		},
		...
		methods: {
			showEditDialog({ _id, name, path, link, shape, note }){
				this.target = {
					_id,
					name,
					path,
			...
			updateLinkEv(){
				UpdateMeal(this.target, {loading: true})
				.then(res => {
			...
			insertLinkEv(){
				AddMeal(this.target, {loading: true})
				.then(res => {
					this.ajaxMsgTips(res);
					if(res.data.code === 200){
						// 重置
			...
			pullData(loadingState = false){
				...
				GetAllMeal(query, {loading: loadingState})
				.then(res => {
					if(res.data.code === 200){
