		...
		<el-drawer
			...
			<div class="moreInfo pub-flex pub-column" style="overflow: hidden;">
				...
				<div class="pub-flex1 cover-row ov-x-h ov-y-a" style="width: 100%;">
					<el-row :gutter="20">
					  	<el-col v-for="item in list" :key="item.path" :span="24">
...
<script>
	import { RemoveMealImg, UploadImages, DirMealImgs } from '@api/meal'
	...
	export default {
		...
		created(){
			this.$Bus.$off('changePickerAdvert');
			this.$Bus.$on('changePickerAdvert', (conf) => {
				if(conf.showState){
					this.pullData({
						cb: () => {
