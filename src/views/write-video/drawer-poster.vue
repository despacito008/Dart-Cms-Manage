<template>
	<div class="video-drawer-poster" time="1578021849735">
		<el-drawer
			title="查看所有海报图片"
			direction="rtl"
			size="550px"
			...
			<div class="moreInfo pub-flex pub-column" style="overflow: hidden;">
				...
				<div class="pub-flex1 poster-row ov-y-a ov-x-h" style="width: 100%;">
					<el-row>
					  	<el-col v-for="item in list" :key="item.path" :span="24" style="padding: 4px;">
...
<script>
	import { DirPosterImgs, UploadImages, RemoveImages } from '@api/video'
	...
	export default {
		...
		created(){
			this.$Bus.$off('changePickerPoster');
			this.$Bus.$on('changePickerPoster', (conf) => {
				if(conf.showState){
					this.pullData({
						cb: () => {
